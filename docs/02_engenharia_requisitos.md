...
| RF-07 (fase contratada) | O sistema deve suportar detecção de anomalias/estados nunca vistos no treino (*open-set*), não só classificação fechada entre as classes conhecidas. | Não implementado na prévia — ver track de pesquisa, [`06_track_pesquisa.md`](06_track_pesquisa.md) H4/H8 |

## 2.2 Requisitos não-funcionais

- **RNF-01 Portabilidade**: o sistema deve rodar em Windows e Linux, arquiteturas x86-64 e ARM64. Desenvolvido em Windows ARM64 e validado continuamente num servidor Linux x86-64 (ver `03_arquitetura.md`, seção "Notas de plataforma") — toda dependência que falha em uma plataforma tem fallback documentado no código, nunca falha silenciosamente.
