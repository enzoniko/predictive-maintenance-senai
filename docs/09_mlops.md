...
| Experimentos | `models/registry.py`: MLflow quando disponível, ou um *tracker* local (JSON + joblib) como fallback (ver `03_arquitetura.md`, seção 3.6, para por que essa dualidade existe). Cada execução registra parâmetros, métricas e o artefato do modelo |
